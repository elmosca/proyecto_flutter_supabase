...
function Copy-UserGuides {
    ...
    $guides = @(
        @{Source = "..\wiki_setup\Guia-Estudiantes.md"; Dest = "Guia-Estudiantes.md"},
        @{Source = "..\wiki_setup\Guia-Tutores.md"; Dest = "Guia-Tutores.md"},
        @{Source = "..\wiki_setup\Guia-Administradores.md"; Dest = "Guia-Administradores.md"}
    )
    
    foreach ($guide in $guides) {
        if (Test-Path $guide.Source) {
            Copy-Item $guide.Source $guide.Dest -Force
            Write-Success "✓ $($guide.Dest) copiado"
        } else {
            Write-Warning "! $($guide.Dest) no encontrado"
        }
    }
